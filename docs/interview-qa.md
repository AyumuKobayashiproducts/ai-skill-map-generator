...
## Q7. なぜ 1on1 練習機能を入れたのですか？

**A.** 「診断結果を読むだけ」だと行動に結びつきにくく、アウトプットの場が必要だと感じたからです。  
スキルマップや求人マッチだけでは「分かった気になる」段階に留まりやすいので、実際に声に出して回答を考える 1on1 練習を通じて、

- 面接本番に近い形で思考を整理する
- AI マネージャーから具体的なフィードバックを受け取る
- セッション履歴とスコアの推移から「話す力」の成長を可視化する

という、「行動→フィードバック→成長」のサイクルを回せるようにしました。

---

## Q8. Usage ダッシュボードで何を見ているのですか？

**A.** 「どの機能がどれだけ使われているか」をざっくり把握し、次にどこを改善するべきかを判断するために使っています。

- 日付ごとのアクセス傾向や、診断・JobMatch・1on1 などイベント名の分布から、ユーザーの利用パターンを把握
- 1on1 セッション数や全体のイベント数を見て、「診断で止まっていないか」「面接練習まで進めているか」を確認
- 障害発生時や大きな変更の直後に、イベント数の急減・偏りがないかをチェック

まだ簡易的なメトリクスですが、「ログを取って終わり」ではなく、ダッシュボードとして**自分で見に行ける形にしたこと**を重視しています。

---

## Q9. なぜ PWA（ホーム画面インストール対応）にしたのですか？

**A.** このアプリは「転職準備の期間中、何度も使ってもらう」ことを想定しているため、ブラウザタブの中に埋もれさせず、アプリのようにホーム画面からすぐ開ける体験を重視しました。

- iOS / Android のホーム画面にアイコンとして追加できるようにし、「ふと思い立ったときに 1 タップで開ける」状態を用意
- シンプルな Service Worker で、基本的なキャッシュや起動の速さも意識
- 完全なオフライン対応ではなく、「日常的に使う Web アプリをアプリライクに扱う」ことをゴールに設計

これにより、「ポートフォリオとしての見栄え」だけでなく、実際の利用シーンに寄り添ったプロダクト設計であることをアピールしています。
